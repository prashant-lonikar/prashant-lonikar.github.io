...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, I'm Prashant' is in the banner component
	expect(screen.getByText(/Hi, I'm Prashant/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('PrashantLonikar-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/PrashantLonikar-Resume.pdf');
